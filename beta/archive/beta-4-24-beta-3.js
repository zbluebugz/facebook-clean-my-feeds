// ==UserScript==
// @name         FB - Clean my feeds (424-beta-3)
...
/*
    v4.24 :: August 2023
        Fixed issues with News Feed posts selection rules
        Fixed issues with Marketplace item selection rules
...
(async function () {
    ...
    function buildMoppingDialog() {
        ...
        async function saveUserOptions(event, source = 'dialog') {
            ...
            if (VARS.isAF) {
                ...
                for (const el of elements) {
                    if (el.hasAttribute(postAttCPID)) el.removeAttribute(postAttCPID);
                    if (el.hasAttribute(postAttChildFlag)) el.removeAttribute(postAttChildFlag);
                }
                // -- remove classes
    ...
    function nf_scrubTheTabbies() {
        ...
        if (elTabList) {
            ...
            // -- sometimes the stories' container is recreated by fb, so we look for it again ...
            // -- if not found, redo the scrubbing ...
            ...
            if (elStoriesContainer === null) {
                const queryStories = 'a[href*="/stories/create/"]';
                ...
                if (elStories) {
                    if (document.querySelector('div[id="S:1"]') === null) {
                        ...
                        // -- parent is up a few levels ... 7 may be a tad bit high, so settle on 5.
                        const elParent = climbUpTheTree(elStories, 7);
                        if (elParent) {
    ...
    function gf_cleanTheConsoleTable(findItem = 'Suggestions') {
        ...
        for (const asideBox of asideBoxes) {
            ...
            hideFeature(elParent, KeyWords.GF_SUGGESTIONS[VARS.language]);
        }
    }
    ...
    function gf_createSpacerSpan() {
        ...
        elSpacerSpan.innerHTML = '<span><span style="position:absolute;width:1px;height:1px;">&nbsp;</span><span aria-hidden="true"> · </span></span>';
        return elSpacerSpan;
    }
    ...
    function gf_createOpenInNewWindow(newGroupPostLink) {
        ...
        elContainer.appendChild(document.createElement('span').appendChild(elLinkOpenNewWindow));
        return elContainer;
    }
    ...
    function handleMutation(mutation) {
        if (VARS.prevURL !== window.location.href) {
            ...
            // -- isAF = (A)ny (F)eed
            // -- 99% of the time, there's only one added node, so don't bother with looping.
            processAddedNode(mutation.addedNodes[0]);
        }
    }
    function processAddedNode(node) {
        // -- (skipping the first XX mutations as they're not relevant)
        VARS.mutationsCount++;
        if ((VARS.mutationsCount > VARS.mutationsInitSkip) && isValidNode(node)) {
            mopUpFeed();
        }
    }
    function isValidNode(node) {
        // -- only interested in certain elements
        // -- and must have something inside them ...
        const validNodeNames = ['DIV', 'SPAN', 'A', 'IMG', '#text', 'svg'];
        return (
            (validNodeNames.includes(node.nodeName)) &&
            (node.nodeType === Node.ELEMENT_NODE) &&
            (node.innerHTML.length > 128 || node.textContent.length !== 0)
        );
    }
    function mopUpFeed() {
        if (VARS.isNF) {
    ...
    // ** Mutation Observer
    let bodyObserver = new MutationObserver(processMutations);
    ...
    function restartObserver() {
        ...
        // -- and start up the observer again ...
