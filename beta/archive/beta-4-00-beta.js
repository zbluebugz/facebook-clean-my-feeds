...
/*
    v4.?? :: November 2022
        Performance - made less aggressive
    v4.10 :: November 2022
...
(async function () {
    ...
    // ** Mutations processor
    function bodyMutating(mutationList) {
        mutationList.forEach((mutation) => {
            // console.info(log+'mutation:', mutation.type, mutation.addedNodes.length, mutation);
            if (mutation.type === 'childList' & mutation.addedNodes.length > 0) {
                if (VARS.prevURL !== window.location.href) {
                    // - page url has changed ... refresh the bodyObserver.
                    runMO();
                    // - switch back to aggressive mode ...
                    VARS.mutationsMode = 1;
                    // console.info(`${log}runMO(): A/N/G/V/M:`, VARS.isAF, VARS.isNF, VARS.isGF, VARS.isVF, VARS.isMF);
                }
                else if (VARS.isAF) {
                    // -- mutationsMax = when to run the code
                    // -- i.e. don't run the code on every new node added.
                    VARS.mutationsCount++;
                    let runNow = false
                    if (VARS.mutationsMode === 1) {
                        if (VARS.mutationsCount <= VARS.mutationsAggressiveMax) {
                            ...
                            // - slow mode
                            //console.info(log+'bodyMutating() - switching to non-aggressie mode');
                            VARS.mutationsMode = 2;
                            VARS.mutationsCount = 0;
                        }
                    }
                    else {
                        runNow = (VARS.mutationsCount >= VARS.mutationsNonAgressiveMax);
                    }

                    if (runNow === true) {
                        let mnode = mutation.target;
                        ...
                        // -- reset mutations count
                        if ( VARS.mutationsMode === 2) {
                            VARS.mutationsCount = 0;
                        }
                    }
                }
