// ==UserScript==
// @name         FB - Clean my feeds (400 series)
...
(async function () {
    ...
    const VARS = {
        // - mutations speed mode, 0 = start/ignore; 1 = aggressive; 2 = non-aggressive;
        mutationsMode: 0,
        // - mutations counter
        mutationsCount: 0,
        // - how many mutations to ignore @ start (usually non-feed elements)
        mutationsInitSkip: 400,
        // - in mutationsMode = 1 (fast/aggressive), how many mutations to process, before switching to mutationsMode 2
        mutationsAggressiveMax: 150,
        // - in mutationsMode = 2 (non-agressive), which NTH element to run the code
        mutationsNonAgressiveMax: 5,
        ...
        for (let mutation of mutations) {
            if ((mutation.type === 'childList') && (mutation.addedNodes.length > 0)) {
                if (VARS.prevURL !== window.location.href) {
                    ...
                    runMO();
                    // - switch to start/ignore scan mode
                    VARS.mutationsMode = 0;
                }
                else if (VARS.isAF) {
                    // -- isAF := any feed
                    ...
                    try {
                        ...
                        if (['A', 'DIV', 'IMG', 'SPAN', '#text', 'svg'].indexOf(mnode.nodeName) > 0) {
                            if ((mnode.nodeType === Node.ELEMENT_NODE) && ((mnode.innerHTML.length < 129) || (mnode.textContent.length === 0))) {
                                ...
                                VARS.mutationsCount++; // nth mutation
                                let runNow = false; // when to execute the code

                                if (VARS.isMF) {
                                    // -- marketplace - always run in fast/aggressive mode
                                    runNow = (VARS.mutationsCount % 2 === 0);
                                }
                                else if (VARS.mutationsMode === 0) {
                                    if (VARS.mutationsCount > VARS.mutationsInitSkip) {
                                        VARS.mutationsMode = 1;
                                        VARS.mutationsCount = 0;
                                    }
                                    runNow = false;
                                }
                                else if (VARS.mutationsMode === 1) {
                                    // - curerntly in fast/aggressive mode
                                    // -- mutationsAggressiveMax := how many mutations to process while running in fast/aggressive mode
                                    if (VARS.mutationsCount <= VARS.mutationsAggressiveMax) {
                                        runNow = true;
                                    }
                                    else {
                                        // - switch to slow mode
                                        VARS.mutationsMode = 2;
                                        runNow = true;
                                    }
                                }
                                else {
                                    // -- currently in slow mode
                                    // -- mutationsNonAgressiveMax := number of mutations skipped
                                    runNow = (VARS.mutationsCount % VARS.mutationsNonAgressiveMax === 0);
                                }
                                if (runNow === true) {
                                    if (VARS.isNF) {
                                        mopUpTheNewsFeed();
                        ...
                        console.error(log + 'fn bodyMutating() :: error:', mnode, error.message, error.stack);
                    }
                }
            }
        }
