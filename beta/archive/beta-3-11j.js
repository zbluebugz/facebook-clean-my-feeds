...
(function () {
    ...
    const SUGGESTED_PAGES = {
        ...
        keywords: {
            ...
            'fr': 'Suggested Pages', // --- needs translation
        }
    };
    // - Suggested events
    ...
    const EVENTS_YOU_MAY_LIKE = {
        ...
        keywords: {
            ...
            'fr': 'Events you may like', // --- needs translation
        }
    };
    // - Paid partnership
    ...
    // - Becaused you viewed a similar post
    ...
    // - "Join" button/link
    // -- a rather "agressive" rule
    // -- will pickup the "Join" word in the top part of the post
    // -- if you have some of the above rules not enabled and this one is,
    //    then the post will be hidden if this rule is a match.
    const JOIN_GROUP_2 = {
        hideThis: false,
        keywords: {
            'en': 'Join',
    ...
    const FD = {
        ...
        // - News and Groups feeds post's blocks
        //   (posts have 1-4 blocks)
        // -- used by getTextContent()
        ...
        // -------- videoNonFeedQS: 'div[id="watch_feed"] > div > div > div > div:first-of-type',
        videoNonFeedQS: '[id=watch_feed] > div > div:first-of-type > div',
        videNonFeedPostBlock: ':scope > div:first-of-type',
        // -------- document.querySelector('div[data-pagelet="MainFeed"]').previousSibling.querySelector(':scope > div')
    ...
    function getTextContentRR(post) {
        // get text node values of right rail - skip certain text values.
    ...
    function doMoppingRightRail() {
        ...
        if (rrbox) {
          if (!rrbox.classList.contains(FD.cssHide)) {
            let ptexts = getTextContentRR(rrbox);
    ...
    function doMoppingInfoBoxes(post) {
        ...
        let infoBlock;
        if (FD.isNF) {
            // - block 0 = friend posted then commented | shop added
            // - block 1 = title/heading
            // - block 2 = content
            // - block 3 = info box OR comments
            // - block 4 = comments (if no info box)
            blocks = post.querySelectorAll(FD.postBlocksQS);
            minBlocks = 5;
            infoBlock = 3;
        }
        else if (FD.isVF) {
            ...
            blocks = post.querySelectorAll(FD.videoBlockQS);
            minBlocks = 4;
            infoBlock = 2;
        }
        else {
        ...
        if (blocks.length >= minBlocks) {
            ...
            if(INFO_BOXES.hideThis && (INFO_BOXES.paths.length > 0)){
                for (let j = 0, jL = INFO_BOXES.paths.length; j < jL; j++) {
                    ...
                    if (links.length) {
                        let infobox = links[0].parentElement.parentElement.parentElement.parentElement;
                        infobox.setAttribute(postAtt, infobox.innerHTML.length);
                        // pinkify(infobox, '');
                        hide(infobox, 'Information box');
                        break;
                    }
                }
            }
    ...
    function doMoppingMP() {
        ...
        //console.info(log + 'mpblocks: ', mpblocks);
        for (let i = 0, iL = mpblocks.length; i < iL; i++) {
            ...
            //console.info(log + 'mptexts:', mptexts, mptexts.indexOf(sponsoredWordMP));
            if (mptexts.indexOf(FD.sponsoredWordMP) >= 0) {
                ...
                for (let k = 0, kL = mpheadings.length; k < kL; k++) {
                    ...
                    if (mpheading.textContent === FD.sponsoredWordMP) {
                        //console.info(log + 'mpheading:', mpheading);
