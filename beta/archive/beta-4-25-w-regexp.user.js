...
// ==/UserScript==
/*
    nb: "re" is the Regular Expresion version.
...
(async function () {
    ...
    function nf_isBlockedText(post) {
        ...
        const blockedText = findFirstMatchRE(postTexts, VARS.Filters.NF_BLOCKED_TEXT_LC);
        return blockedText;
    }
    ...
    function gf_isBlockedText(post) {
        ...
        const postTexts = (extractTextContent(post, blocksQuery, 3)).join(' ').toLowerCase();
        const blockedText = findFirstMatchRE(postTexts, VARS.Filters.GF_BLOCKED_TEXT_LC);
        return blockedText;
    }
    ...
    function vf_isBlockedText(post, queryBlocks) {
        ...
        const blockedText = findFirstMatchRE(postTexts, VARS.Filters.VF_BLOCKED_TEXT_LC);
        return blockedText;
    }
    ...
    function mp_getBlockedPrices(elBlockOfText) {
        ...
        if (VARS.Filters.MP_BLOCKED_TEXT.length > 0) {
            ...
            const blockedPrices = findFirstMatchRE(itemPrices, VARS.Filters.MP_BLOCKED_TEXT_LC);
    ...
    function mp_getBlockedTextDescription(collectionBlocksOfText, skipFirstBlock = true) {
        ...
        if (VARS.Filters.MP_BLOCKED_TEXT_DESCRIPTION.length > 0) {
            ...
            for (let i = startIndex; i < collectionBlocksOfText.length; i++) {
                ...
                const blockedText = findFirstMatchRE(descriptionText, VARS.Filters.MP_BLOCKED_TEXT_DESCRIPTION_LC);
